                        ...
                        */
                        if (objectStatus === 'M') {
                            ...
                            loadModuleDescription(
                                dir,
                                createObject(cachedObject.type, cachedObject.file),
                                function (object) {
...
function findInDir (rootDir, dir, callback) {
    ...
    abc.find(
        ...
        function (file, dirPath) {
            ...
            if (isModule(file)) {
                objects.push(createModule(filePath));
            } else if (isPackage(file)) {
                objects.push(createPackage(filePath));
            }
        },
        function () {
            abc.async.forEach(
...
function createModule (file) {
    return createObject('module', file)
}

function createPackage (file) {
    return createObject('package', file)
}
